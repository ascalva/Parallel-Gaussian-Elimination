        ...
        double  *b_og )
{
    ...
    // Makes copies of the values of matrix A and vector B. Computing Gaussial Elimination
    // will require in-place modifications to the matrices, such that we need to have back
    // up copies of the originals to check if the solution is correct.
    for (col = 0; col < N; col++) {
        for (row = 0; row < N; row++) {
...
//
// NOTE: Not required for parallel program to work, just a simple sanity check.
void check_comp(int N, double (*A)[N], double *X, double *result)
{
    ...
    for( row = 0; row < N; row++ )
    {
        ...
        // Check if computed result is equal to original result.
        // NOTE: May result in failures due to roundint errors, this is 
        //       to be expected, as result should be an approximation of the
        //       actual values, not an analytical, exact result.
        printf(" %5.2f   %5.2f   %s\n", result[row], res, 
...
int main(int argc, char *argv[])
{
    ...
    // Error checking for CL arguments.
	if (argc != 3)
	{
    ...
    // End timer.
    printf("test: %f\n", ((double)(clock() - c_begin_exec)) / CLOCKS_PER_SEC);
    c_end_exec = clock();
    ...
    printf("Time taken to compute vector:      %3.4f (s)\n",
            ((double)(c_end_exec - c_begin_exec)) / (CLOCKS_PER_SEC * n));
